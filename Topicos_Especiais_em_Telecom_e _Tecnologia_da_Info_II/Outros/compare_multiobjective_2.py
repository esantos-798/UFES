...
def run_mohaf_ultra(problem_name, max_iter=100, num_pop=30, max_archive=50, p_turbulento=0.15):
    ...
    for t in range(max_iter):
        ...
        # Coeficiente de escoamento logarítmico do HAF Ultra original
        a = 2.0 * np.log(1.0 + (max_iter - t) / max_iter) / np.log(2.0)
        
        for i in range(num_pop):
            
            # -----------------------------------------------------------------
            # GATILHO REORGANIZADOR: REGIME DE ESCOAMENTO TURBULENTO
            # -----------------------------------------------------------------
            if np.random.rand() < p_turbulento:
                # O turbilhonamento gera vetores de vórtice aleatórios baseados em Cauchy.
                # A escala do passo reduz proporcionalmente à convergência do algoritmo.
                escala_vortice = 0.02 * (ub - lb) * (1.0 - (t / max_iter))
                X[i] = X[i] + np.random.standard_cauchy(D) * escala_vortice
                
            else:
                # -----------------------------------------------------------------
                # REGIME LAMINAR ORIGINAL: MOTOR GEOMÉTRICO HAF ULTRA
                # -----------------------------------------------------------------
            ...
            # Restrições de fronteira física
...
# [Defina aqui a sua função principal adaptada: run_mohaf_ultra]
