...
def make_box_lines(cx, cy, cz, dx, dy, dz):
    """Paralelepípedo como polilinha contínua com NaN separando arestas."""
    x0, x1 = cx-dx, cx+dx
    y0, y1 = cy-dy, cy+dy
    z0, z1 = cz-dz, cz+dz
    ...
    return np.array(pts).T  # shape (3, N)

def make_cylinder_lines(cx, cy, cz_bot, cz_top, radius, segments=8):
    """Cilindro como polilinha contínua com NaN separando partes."""
    angles = np.linspace(0, 2*np.pi, segments+1)
    nan = [float('nan'), float('nan'), float('nan')]
    pts = []
    for a in angles:
        pts.append([cx+radius*np.cos(a), cy+radius*np.sin(a), cz_bot])
    pts.append(nan)
    for a in angles:
        pts.append([cx+radius*np.cos(a), cy+radius*np.sin(a), cz_top])
    pts.append(nan)
    for a in angles[::2]:
        pts.append([cx+radius*np.cos(a), cy+radius*np.sin(a), cz_bot])
        pts.append([cx+radius*np.cos(a), cy+radius*np.sin(a), cz_top])
...
def concatenate_parts(*parts):
    """Junta polilinhas com separador NaN entre elas."""
...
# Linha homogênea: 1 onde há ponto real, NaN onde há NaN (matmul ignora NaN
# corretamente se tratarmos depois) — solução: substituir NaN por 0 só na
# linha homogênea, e manter NaN nas coords para o plot funcionar
    hom_row = np.where(np.isnan(robot_xyz[0, :]), np.nan, 1.0)
    return np.vstack([robot_xyz, hom_row])


# Define the intrinsic parameters as separate variables so you can chance each
# one of them later if you want
#f = 20  #Changed to 20 for faciliting the robot view in the last activite.
#w_cco = 36
#h_cco = 24
#w_pixels = 1280
#h_pixels = 720
...
#K = np.array([[f*sx,skew_factor,ox],[0,f*sy,oy],[0,0,1]])

# Print the intrinsic parameter matrix

#G = np.linalg.inv(cam3)
...
# Projecting the robot onto the image plane
...
# Plot the project points of the robot to see the image
...
#ax2.plot3D(robot[0,:], robot[1,:], robot[2,:], 'red')
# Ploting the robot corner as a reference
#ax2.scatter(robot[0,0], robot[1,0], robot[2,0],'b')
...
# Create a function to perform the perspective projection
...
# Plot the 3D scene and the new image generated by the camera.
...
class CameraApp:
    ...
    def _build_ui(self):
        ...
        fc  = '#e8e8e8'   # fundo dos campos
        tc  = '#222222'   # cor do texto
        lc  = '#1a5276'   # cor dos labels
        ...
        def button(row, col, text, colspan=1):
            ax = self.fig.add_subplot(gs_ctrl[row, col:col+colspan])
            b  = Button(ax, text, color=btn, hovercolor='#444444')
            b.label.set_color(tc); b.label.set_fontsize(8)
            return b

        # ── Título painel ──
        lbl(0, 0, '🎥  Controles da Câmera', color='#4a235a')
        ...
        lbl(4, 0, 'dx', colspan=1, color='#c0392b')
        self.tb_dx = textbox(4, 1, '0')
        lbl(5, 0, 'dy', colspan=1, color='#1a7a40')
        self.tb_dy = textbox(5, 1, '0')
        lbl(6, 0, 'dz', colspan=1, color='#154360')
        ...
        lbl(10, 0, 'Rx', colspan=1, color='#c0392b')
        self.tb_rx = textbox(10, 1, '0')
        lbl(11, 0, 'Ry', colspan=1, color='#1a7a40')
        self.tb_ry = textbox(11, 1, '0')
        lbl(12, 0, 'Rz', colspan=1, color='#154360')
        ...
        self.btn_reset = Button(ax_reset, '↺ Reset', color='#e8e8e8', hovercolor='#444444')
        self.btn_info  = Button(ax_info,  'ℹ Info',  color='#e8e8e8', hovercolor='#444444')
        ...
        self.ax_status.set_facecolor('#ffffff'); self.ax_status.axis('off')
        self.txt_status = self.ax_status.text(
            0.5, 0.5, 'Pronto.', ha='center', va='center',
            color='#196f3d', fontsize=8, transform=self.ax_status.transAxes,
            wrap=True)
