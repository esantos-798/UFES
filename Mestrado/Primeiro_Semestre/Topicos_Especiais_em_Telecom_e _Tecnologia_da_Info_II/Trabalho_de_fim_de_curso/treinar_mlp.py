...
def carregar_dados_multivariados(caminho_arq):
    """
    Lê o arquivo mapeando as colunas pela ordem física (índices),
    evitando falhas de string e caracteres invisíveis nos cabeçalhos.
    """
    ...
    print(f"    Mapeamento físico -> Time:{idx_time} | Current:{idx_current} | Power:{idx_power} | OSNR:{idx_osnr} | BER:{idx_ber}")
    ...
    for chunk in chunks:
        ...
        c_time = chunk.columns[idx_time]
        c_current = chunk.columns[idx_current]
        c_power = chunk.columns[idx_power]
        c_osnr = chunk.columns[idx_osnr]
        c_ber = chunk.columns[idx_ber]
...
def construir_features_sequenciais(df, window_size=10):
    """
    Garante a engenharia de features (lags) agrupando estritamente por Lightpath_ID,
    calculando o TTF de forma isolada por trajetória (evitando data leakage).
    """
    X_list, y_list = [], []
...
if __name__ == "__main__":
    ...
    print("1. Carregando dados brutos e mapeando trajetórias físicas...")
    df_train_raw = carregar_dados_multivariados(arq_train)
    df_test_raw = carregar_dados_multivariados(arq_test)
    
    print("\n2. Dividindo dataset por trajetórias isoladas (Garantia anti-leakage)...")
    print("    Processando janelas temporais de 10 passos...")
    ...
    del df_train_raw, df_test_raw  # Liberação cirúrgica de RAM
    
    print("\n3. Aplicando normalização estatística escalonada...")
    scaler = StandardScaler()
    X_train = scaler.fit_transform(X_train)
    X_test = scaler.transform(X_test)
    
    print(f"   Matriz Treino: {X_train.shape} | Matriz Teste: {X_test.shape}")
    
    print("\n4. Treinando a Rede Neural Multi-Layer Perceptron (Escala de Trajetórias)...")
    mlp = MLPRegressor(
    ...
    print("\n5. Avaliando predições e calculando erro de degradação...")
    predicoes = mlp.predict(X_test)
    mae_mlp = mean_absolute_error(y_test, predicoes)
    
    print(f"\n[SUCESSO] MAE obtido na Rede Neural com divisão anti-leakage: {mae_mlp:.2f} passos de tempo.")