...
# Versão: adiciona StandardScaler e corrige cap de avaliação (MAE_approach)
...
if __name__ == "__main__":
    ...
    # 2. Cálculo do TTF (Igualando os caps para evitar distorção de escala se necessário)
    df_tr_raw = calcular_ttf(df_tr_raw, cap=TTF_CAP_TRAIN)
    df_te_raw = calcular_ttf(df_te_raw, cap=TTF_CAP_TEST)

    # 3. Split de IDs apenas para o arquivo de treino (separando Treino e Validação interna)
    # Mudamos o test_frac para 0.0 porque o teste real virá do outro arquivo externo
    lp_tr, lp_val, _ = split_lightpath_ids(df_tr_raw, val_frac=0.15, test_frac=0.0)
    
    # Todos os IDs do arquivo de teste original entram como teste
    lp_te = set(df_te_raw["lightpath_id"].unique())

    # 4. Extração com subsampling alimentando as duas fontes brutas separadamente
    ...
    # Unifica no dicionário final de execução
    data = {
    ...
    del df_tr_raw, df_te_raw, data_train_val, data_test  # Liberação cirúrgica de memória RAM

    # -------------------------
    # StandardScaler (fit só no treino) - TRANSFORMA data inplace-like
    # -------------------------
    print("\nAplicando StandardScaler nas features (fit apenas em treino)...")
    scaler = StandardScaler().fit(data["train"][0])
    def transform_split(splt):
        X, y, ft = data[splt]
        Xs = scaler.transform(X)
        return (Xs, y, ft)
    data = {s: transform_split(s) for s in ("train", "val", "test")}
    ...
    for seed in range(N_SEEDS):
        t0 = time.time()
        # Atenção: aqui passamos o cap de TESTE para calcular MAE_approach corretamente
        mae, rmse, r2, mae_apr, rf, y_te, y_pr, ft_te = avaliar(
            data, seed, cap=TTF_CAP_TEST)
