...
zdt_benchmarks = ["zdt1", "zdt2", "zdt3", "zdt4", "zdt6"] 
fig = plt.figure(figsize=(18, 10))

print("Gerando Plots Comparativos das Fronteiras 2D (Completo - 5 Problemas)...")
for idx, b in enumerate(zdt_benchmarks):
    prob = get_problem(b)
    ref_dirs = get_reference_directions("das-dennis", prob.n_obj, n_partitions=99)
    
    mohaf_pf = run_mohaf_ultra(b, max_iter=500, num_pop=100)
    ...
    # Organiza em uma grade de 2 linhas e 3 colunas
    ax = fig.add_subplot(2, 3, idx+1)
    
    ax.plot(pf_teorico[:, 0], pf_teorico[:, 1], color='black', label='Fronteira Teórica', linewidth=1.5, alpha=0.5)
    
    if len(mohaf_pf) > 0: 
        ax.scatter(mohaf_pf[:, 0], mohaf_pf[:, 1], color='blue', s=25, label='MOHAF Ultra', marker='o')
    ax.scatter(res_nsga3[:, 0], res_nsga3[:, 1], color='orange', s=15, label='NSGA-III', marker='x')
    ax.scatter(res_moead[:, 0], res_moead[:, 1], color='green', s=15, label='MOEA/D', marker='^')
    
    # Tratamento de escala para o ZDT4 não estourar o gráfico
    if b == "zdt4":
