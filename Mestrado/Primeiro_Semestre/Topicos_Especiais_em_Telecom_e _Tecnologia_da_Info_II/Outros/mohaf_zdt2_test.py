...
# =============================================================================
# 1. BENCHMARK MULTI-OBJETIVO (ZDT2) E AUXILIARES DE PARETO
# =============================================================================
def zdt2(x):
    f1 = x[0]
    g = 1.0 + 9.0 * np.sum(x[1:]) / (len(x) - 1)
    # A mudança para o quadrado da fração gera a concavidade na fronteira
    f2 = g * (1.0 - (f1 / g)**2)
...
def run_mohaf(max_archive=50):
    ...
    num_pop = 30
    max_iter = 100
    bounds = [0.0, 1.0] # Domínio padrão do ZDT2
    
    # Inicialização da População
    X = np.random.uniform(bounds[0], bounds[1], size=(num_pop, D))
    X_costs = np.array([zdt2(ind) for ind in X])
...
# =============================================================================
# 3. EXECUÇÃO E VISUALIZAÇÃO DA FRONTEIRA DE PARETO (ZDT2)
# =============================================================================
print("Executando o MOHAF v1.0 no benchmark côncavo ZDT2...")
# Lembre-se de mudar a chamada interna na run_mohaf para zdt2!
fronteira_obtida = run_mohaf(max_archive=60) 

# Curva teórica do ZDT2
f1_teorico = np.linspace(0, 1, 200)
f2_teorico = 1.0 - f1_teorico**2

plt.figure(figsize=(8, 6))
plt.plot(f1_teorico, f2_teorico, color='red', linestyle='--', label='Fronteira Teórica Côncava')
plt.scatter(fronteira_obtida[:, 0], fronteira_obtida[:, 1], color='blue', edgecolors='k', s=45, label='MOHAF')
plt.title('MOHAF enfrentando a Fronteira Côncava do ZDT2', fontsize=12, fontweight='bold')
