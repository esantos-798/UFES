...
class LSTNetModificado(nn.Module):
    def __init__(self, window, hidRNN, hidCNN, CNN_kernel, skip, hidSkip, num_features):
        ...
        # 4. Componente Autoregressiva (AR) dedicada por variável
        self.ar = nn.Linear(window, 1)
        
        # 5. Camada de Fusão Totalmente Conectada do Artigo
        self.output_layer = nn.Linear(num_features, num_features)
        
        # Inicialização uniforme de Xavier nos pesos para evitar saturação
        self._init_weights()
    ...
    def forward(self, x):
        ...
        # --- 1. PROCESSAMENTO DEEP (CNN + GRU) ---
        c = x.view(-1, 1, self.window, self.num_features)
        c = F.leaky_relu(self.conv1(c), negative_slope=0.01)
        c = torch.squeeze(c, 3).permute(0, 2, 1)
        
        _, r = self.GRU1(c)
        r = r[-1, :, :]  # Mantém dimensão do lote intacta
        ...
        # --- 2. PROCESSAMENTO LINEAR INDEPENDENTE (AR POR FEATURE) ---
        x_ar_list = []
        for i in range(self.num_features):
            feat_series = x[:, :, i]  # Isola a série temporal da feature 'i' (batch, window)
            feat_pred = self.ar(feat_series)  # Passa pela subcamada linear dedicada (batch, 1)
            x_ar_list.append(feat_pred)
            
        x_ar = torch.cat(x_ar_list, dim=1)  # Reconstrói a matriz (batch, num_features)
        
        # --- 3. FUSÃO NÃO-LINEAR DO ARTIGO ---
        # h_out = neural_out + x_ar  <-- comente a original
        h_out = neural_out           # <-- use apenas a parte profunda
        return F.leaky_relu(self.output_layer(h_out), negative_slope=0.01)
