...
% Garantir toolkit gráfico moderno e carregar pacote de otimização
graphics_toolkit('qt');
suppress_warnings = true;

% Carrega pacote de otimização se estiver disponível (necessário para lsqcurvefit)
if exist('pkg', 'file')
    try
        pkg load optim;
    catch
        % Se não estiver instalado, avisa o usuário
        warning('O pacote "optim" não está carregado. Instale executando: pkg install -forge optim');
    end
end
...
% --- AJUSTE COMPATÍVEL COM OCTAVE (Substituição do histcounts) ---
t_bins = t0 : passo_s : t_limite;
t_centro = t_bins(1:end-1) + passo_s/2;

% Utiliza os centros das caixas diretamente na função hist nativa
if isempty(t_chegada_valido)
    contagens = zeros(1, length(t_centro));
else
    contagens = hist(t_chegada_valido, t_centro);
end
...
% Configuração de otimização adaptada para o Octave
try
    options = optimset('Display', 'iter', 'TolFun', 1e-12);
    C_finais = lsqcurvefit(dgf_norm, C0, t_data_ns, h_mc_norm, lb, ub, options);
catch
    warning('Falha ao executar lsqcurvefit. Certifique-se de que o pacote "optim" está instalado.');
    C_finais = C0; % Fallback
end
...
delta_t_segundos = t_centro - t0_min;
